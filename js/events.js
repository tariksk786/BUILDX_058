...
export const EventTypes = {
  ...
  EMERGENCY_PREARRIVAL_CREATED: 'EMERGENCY_PREARRIVAL_CREATED',
  ...
  // TRUST CARE Core Domain Events
  TRUSTCARE_CASE_CREATED: 'TRUSTCARE_CASE_CREATED',
  HOSPITAL_REQUEST_SENT: 'HOSPITAL_REQUEST_SENT',
  HOSPITAL_ACCEPTED: 'HOSPITAL_ACCEPTED',
  HOSPITAL_DECLINED: 'HOSPITAL_DECLINED',
  ACCEPTANCE_WITHDRAWN: 'ACCEPTANCE_WITHDRAWN',
  HOSPITAL_REQUEST_TIMEOUT: 'HOSPITAL_REQUEST_TIMEOUT',
  RESOURCE_HELD: 'RESOURCE_HELD',
  RESOURCE_HOLD_REJECTED: 'RESOURCE_HOLD_REJECTED',
  RESOURCE_HOLD_CONFIRMED: 'RESOURCE_HOLD_CONFIRMED',
  RESOURCE_HOLD_RELEASED: 'RESOURCE_HOLD_RELEASED',
  RESOURCE_HOLD_EXPIRED: 'RESOURCE_HOLD_EXPIRED',
  READINESS_UPDATED: 'READINESS_UPDATED',
  CASE_BLOOD_REQUESTED: 'CASE_BLOOD_REQUESTED',
  CASE_BLOOD_ACKNOWLEDGED: 'CASE_BLOOD_ACKNOWLEDGED',
  CASE_BLOOD_CONFIRMED: 'CASE_BLOOD_CONFIRMED',
  CASE_BLOOD_RESERVED: 'CASE_BLOOD_RESERVED',
  CASE_BLOOD_UNAVAILABLE: 'CASE_BLOOD_UNAVAILABLE',
  PATIENT_ARRIVED: 'PATIENT_ARRIVED',
  HANDOVER_IN_PROGRESS: 'HANDOVER_IN_PROGRESS',
  HANDOVER_COMPLETED: 'HANDOVER_COMPLETED',
  DOCTOR_EMERGENCY_DUTY_ASSIGNED: 'DOCTOR_EMERGENCY_DUTY_ASSIGNED',
...
export const EventDescriptions = {
  ...
  [EventTypes.PATIENT_IN_ROOM]: (e) => `Patient ${e.payload.patientName || 'Patient'} entered consultation room with Dr. ${e.payload.doctorName || 'Doctor'}`,
  ...
  [EventTypes.EMERGENCY_PREARRIVAL_CREATED]: (e) => `Incoming Emergency Pre-Arrival: ${e.payload.patientName || 'Emergency Patient'} via ${e.payload.transportMode || 'Private Vehicle'} (ETA ~${e.payload.etaMinutes || 14} min)`,
  ...
  [EventTypes.MEDICATION_SKIPPED]: (e) => `${e.payload.patientName || 'Patient'} skipped medication dose: ${e.payload.medicationName} (Reason: ${e.payload.reason || 'Not specified'})`,
  ...
  // TRUST CARE Descriptions
  [EventTypes.TRUSTCARE_CASE_CREATED]: (e) => `🚨 Emergency Case ${e.payload.caseObj?.id || e.entityId || 'TC-014'} logged with verified requirements`,
  [EventTypes.HOSPITAL_REQUEST_SENT]: (e) => `Emergency transmission sent to ${e.payload.hospitalId} (Awaiting confirmation)`,
  [EventTypes.HOSPITAL_ACCEPTED]: (e) => `✅ ${e.payload.hospitalId} ACCEPTED emergency case. ${e.payload.resourceId || 'ICU'} held.`,
  [EventTypes.HOSPITAL_DECLINED]: (e) => `⚠️ ${e.payload.hospitalId} DECLINED emergency case. Reason: ${e.payload.reason}`,
  [EventTypes.ACCEPTANCE_WITHDRAWN]: (e) => `🚨 CRITICAL: ${e.payload.hospitalId} WITHDREW acceptance due to: ${e.payload.reason}. Re-routing required!`,
  [EventTypes.HOSPITAL_REQUEST_TIMEOUT]: (e) => `⏱️ Request to ${e.payload.hospitalId} timed out. Re-routing recommended.`,
  [EventTypes.RESOURCE_HELD]: (e) => `🔒 Resource ${e.payload.resourceId} placed on atomic operational hold for ${e.payload.caseId}`,
  [EventTypes.RESOURCE_HOLD_REJECTED]: (e) => `⛔ Double-allocation prevented: ${e.payload.message}`,
  [EventTypes.RESOURCE_HOLD_CONFIRMED]: (e) => `Resource ${e.payload.resourceId} hold confirmed on clinical admission`,
  [EventTypes.RESOURCE_HOLD_RELEASED]: (e) => `Resource ${e.payload.resourceId} hold released`,
  [EventTypes.RESOURCE_HOLD_EXPIRED]: (e) => `Resource hold expired for case ${e.payload.caseId}`,
  [EventTypes.READINESS_UPDATED]: (e) => `Pre-arrival checklist: ${e.payload.itemKey} is ${e.payload.status} (Overall: ${e.payload.overall})`,
  [EventTypes.CASE_BLOOD_REQUESTED]: (e) => `Case-linked blood request logged: ${e.payload.request?.unitsRequested} units ${e.payload.request?.bloodGroup}`,
  [EventTypes.CASE_BLOOD_ACKNOWLEDGED]: (e) => `Blood bank acknowledged emergency requirement for Case ${e.payload.caseId}`,
  [EventTypes.CASE_BLOOD_CONFIRMED]: (e) => `Blood bank confirmed physical availability for Case ${e.payload.caseId}`,
  [EventTypes.CASE_BLOOD_RESERVED]: (e) => `🩸 Blood units reserved and prepped for emergency bay (Case ${e.payload.caseId})`,
  [EventTypes.CASE_BLOOD_UNAVAILABLE]: (e) => `⚠️ Blood stock unavailable for Case ${e.payload.caseId}. Regional escalation triggered.`,
  [EventTypes.PATIENT_ARRIVED]: (e) => `🚑 Ambulance arrived at ${e.payload.bayNumber || 'Emergency Bay'}. Physical transport complete.`,
  [EventTypes.HANDOVER_IN_PROGRESS]: (e) => `Bedside SBAR clinical handover in progress with ${e.payload.doctorName}`,
  [EventTypes.HANDOVER_COMPLETED]: (e) => `🎯 CONFIRMED HANDOVER COMPLETED. Clinical responsibility transferred to ${e.payload.doctorName}.`,
  [EventTypes.DOCTOR_EMERGENCY_DUTY_ASSIGNED]: (e) => `Physician ${e.payload.doctorName} assigned to emergency duty. OPD queue wait time updated (+${e.payload.delayMinutes}m).`
};
...
export function getEventIcon(eventType) {
  const icons = {
    ...
    // TRUST CARE icons
    [EventTypes.TRUSTCARE_CASE_CREATED]: 'fa-truck-medical',
    [EventTypes.HOSPITAL_REQUEST_SENT]: 'fa-paper-plane',
    [EventTypes.HOSPITAL_ACCEPTED]: 'fa-circle-check',
    [EventTypes.HOSPITAL_DECLINED]: 'fa-circle-xmark',
    [EventTypes.ACCEPTANCE_WITHDRAWN]: 'fa-triangle-exclamation',
    [EventTypes.HOSPITAL_REQUEST_TIMEOUT]: 'fa-hourglass-end',
    [EventTypes.RESOURCE_HELD]: 'fa-lock',
    [EventTypes.RESOURCE_HOLD_REJECTED]: 'fa-ban',
    [EventTypes.RESOURCE_HOLD_CONFIRMED]: 'fa-bed-pulse',
    [EventTypes.READINESS_UPDATED]: 'fa-list-check',
    [EventTypes.CASE_BLOOD_REQUESTED]: 'fa-droplet',
    [EventTypes.CASE_BLOOD_CONFIRMED]: 'fa-vial-circle-check',
    [EventTypes.CASE_BLOOD_RESERVED]: 'fa-box-archive',
    [EventTypes.PATIENT_ARRIVED]: 'fa-hospital',
    [EventTypes.HANDOVER_IN_PROGRESS]: 'fa-user-nurse',
    [EventTypes.HANDOVER_COMPLETED]: 'fa-shield-halved',
    [EventTypes.DOCTOR_EMERGENCY_DUTY_ASSIGNED]: 'fa-stethoscope',
