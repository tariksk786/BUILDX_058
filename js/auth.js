...
const Auth = {
  ...
  async login(email, password, roleHint = null) {
    ...
    if (!Config.IS_DEMO && this.supabase && password) {
      try {
        const { data, error } = await this.supabase.auth.signInWithPassword({ email: cleanEmail, password });
        if (error) throw new Error(error.message);
      } catch (err) {
        console.warn('[Auth] Supabase direct auth note:', err.message);
      }
    }

    // 2. Find profile in local registry / demo users
    const user = this._findUserByEmail(cleanEmail);
    if (!user) {
      throw new Error(`No account found for "${cleanEmail}". Please check your credentials or select a Demo Role.`);
    }

    if (user.accountStatus && user.accountStatus !== 'active') {
      throw new Error('Your account is inactive. Please contact system administration.');
    }

    // 3. Optional role hint verification
    if (roleHint && user.role !== roleHint && user.role !== 'admin') {
      const error = new Error(`Account "${cleanEmail}" is registered as "${user.role}", not "${roleHint}".`);
      error.roleMismatch = true;
      error.correctRole = user.role;
      error.redirectUrl = this.getRoleHome(user.role);
      throw error;
    }
  ...
  /**
   * Fast Demo Access for Hackathon Verification
   * Authenticates into verified identity and establishes authoritative session
   */
  loginDemo(role) {
    const cleanRole = (role || 'dispatcher').toLowerCase();
    const demoMap = {
      dispatcher: {
        id: 'u-dispatcher',
        email: 'dispatcher@trustcare.org',
        displayName: 'Aarav Mehta',
        role: 'dispatcher',
        department: 'Central Emergency Dispatch',
        organization: 'Central EMS Control',
        badge: 'EMS-DISPATCH-07',
        isDemo: true
      },
      hospital_desk: {
        id: 'u-hosp-desk',
        email: 'emergency.desk@metrogeneral.org',
        displayName: 'Metro Emergency Desk Coordinator',
        role: 'hospital_desk',
        department: 'Emergency Receiving Desk',
      ...
      blood_bank: {
        id: 'u-blood-desk',
        email: 'blood.desk@metrogeneral.org',
        displayName: 'Metro Blood Bank Lead',
        role: 'blood_bank',
        department: 'Blood Transfusion Service',
        organization: 'Metro General Blood Bank',
        badge: 'BLOOD-LEAD-03',
        isDemo: true
      },
      admin: {
        id: 'u-admin',
        email: 'admin@trustcare.org',
        displayName: 'TRUST CARE Operations Director',
        role: 'admin',
        department: 'System Administration & Governance',
        organization: 'State Emergency Authority',
        badge: 'SYS-ADMIN-01',
        isDemo: true
      },
      public: {
        id: 'u-public',
        email: 'caller@emergency.org',
        displayName: 'Public Caller (Citizen)',
        role: 'public',
        department: 'Emergency Assistance Requester',
        phone: '+91 9811223344',
        isDemo: true
      },
      doctor: {
        id: 'u-doc-1',
        email: 'dr.sharma@hospitalflow.ai',
        displayName: 'Dr. Aarav Sharma',
        role: 'doctor',
        department: 'General Medicine & Trauma Lead',
        doctorId: 'D-0001',
        isDemo: true
      }
    };

    const targetUser = demoMap[cleanRole];
    if (!targetUser) {
      throw new Error(`Unknown demo role: "${role}". Valid roles: dispatcher, hospital_desk, blood_bank, admin, public, doctor`);
    }

    this._setCurrentUser(targetUser);
    Storage.saveAuth(targetUser);
    ...
    eventBus.emit(EventTypes.USER_LOGGED_IN, {
      ...
      user: targetUser,
      isDemo: true
    }, { source: 'demo_auth', userId: targetUser.id });

    return targetUser;
  },

  /**
   * Get Home Dashboard Route for Any Role
   */
  getRoleHome(role) {
    const map = {
    ...
    return map[role] || '/dispatcher';
  },

  /**
   * Strict Role-Based Route Authorization Gatekeeper
   */
  canAccessRoute(route, user) {
    // 1. Always public routes
    if (!route || route === '/' || route === '/login' || route === '/emergency-request' ||
        route === '/demo' || route.startsWith('/patient/login') || route.startsWith('/doctor/login') || route.startsWith('/admin/login')) {
      return { allowed: true };
    }

    // 2. Unauthenticated user trying to access protected operational routes
    if (!user || !user.role) {
      return { allowed: false, reason: 'unauthenticated', redirect: '/login' };
    }

    // 3. Admin has access to all operational views
    if (user.role === 'admin') return { allowed: true };

    // 4. Role-specific route boundaries
    if (route === '/dispatcher' || route.startsWith('/dispatcher/')) {
      if (user.role === 'dispatcher') return { allowed: true };
      return { allowed: false, reason: 'role_mismatch', redirect: this.getRoleHome(user.role) };
    }

    if (route === '/hospital-desk' || route.startsWith('/hospital-desk/')) {
      if (user.role === 'hospital_desk') return { allowed: true };
      return { allowed: false, reason: 'role_mismatch', redirect: this.getRoleHome(user.role) };
    }

    if (route === '/blood-bank' || route.startsWith('/blood-bank/')) {
      if (user.role === 'blood_bank') return { allowed: true };
      return { allowed: false, reason: 'role_mismatch', redirect: this.getRoleHome(user.role) };
    }

    if (route === '/timeline' || route.startsWith('/timeline/')) {
      // Timeline is accessible to all authenticated operational roles
      return { allowed: true };
    }

    if (route.startsWith('/doctor/')) {
      if (user.role === 'doctor' || user.role === 'hospital_desk') return { allowed: true };
      return { allowed: false, reason: 'role_mismatch', redirect: this.getRoleHome(user.role) };
    }

    if (route.startsWith('/patient/')) {
      if (user.role === 'patient' || user.role === 'public') return { allowed: true };
      return { allowed: false, reason: 'role_mismatch', redirect: this.getRoleHome(user.role) };
    }
  ...
  /**
   * Authoritative Action Permission Validator (TRUST CARE Security Matrix)
   * Validates whether an actor has permission to perform an operational action.
   */
  can(action, customUser = null) {
    const user = customUser || this.getCurrentUser();
    if (!user || !user.role) return false;

    // Admin has universal operational oversight
    if (user.role === 'admin') return true;

    const permissions = {
      // ---- TRUST CARE Core Operational Matrix ----
      // 1. Dispatcher Actions
      'create_emergency_case': ['dispatcher'],
      'send_hospital_request': ['dispatcher'],
      'update_ambulance_status': ['dispatcher'],
      'verify_public_request': ['dispatcher'],

      // 2. Hospital Emergency Desk Actions
      'accept_hospital_request': ['hospital_desk'],
      'decline_hospital_request': ['hospital_desk'],
      'withdraw_hospital_acceptance': ['hospital_desk'],
      'create_resource_hold': ['hospital_desk'],
      'release_resource_hold': ['hospital_desk'],
      'extend_resource_hold': ['hospital_desk'],
      'acknowledge_receiving_team': ['hospital_desk', 'doctor'],
      'start_handover': ['hospital_desk'],
      'confirm_handover': ['hospital_desk'],

      // 3. Blood Bank Desk Actions
      'acknowledge_blood_request': ['blood_bank'],
      'confirm_blood_availability': ['blood_bank'],
      'reserve_blood': ['blood_bank'],
      'flag_blood_unavailable': ['blood_bank'],
      'mark_blood_received': ['hospital_desk', 'blood_bank'],

      // Shared Operational Handshake Actions
      'mark_ambulance_arrival': ['dispatcher', 'hospital_desk'],
      'view_audit_logs': ['dispatcher', 'hospital_desk', 'blood_bank', 'doctor', 'patient'],

      // Public / Emergency Caller
      'request_emergency_assistance': ['public', 'patient', 'dispatcher'],
      ...
      'patient.emergency.request': ['patient', 'public'],
      'patient.ambulance.request': ['patient', 'public'],
      ...
      'check_in_patient': ['patient', 'doctor'],
      'manage_queue': ['doctor'],
      'call_patient': ['doctor'],
      'start_consultation': ['doctor'],
      'complete_consultation': ['doctor'],
      'insert_emergency': ['doctor', 'dispatcher'],
      'change_doctor_status': ['doctor'],
      'transfer_patient': ['doctor'],
      'mark_no_show': ['doctor'],
      'create_blood_request': ['doctor', 'dispatcher'],
      'issue_blood': ['blood_bank'],
      'manage_donors': ['blood_bank'],
      'create_discharge': ['doctor'],
      'create_followup': ['doctor'],
      'view_own_care': ['patient', 'doctor']
    };
  ...
  /**
   * Assert permission or throw descriptive Authorization Error
   */
  assertCan(action, customUser = null) {
    const user = customUser || this.getCurrentUser();
    if (!user || !user.role) {
      const err = new Error(`[Authorization Error] Unauthenticated: Action "${action}" requires an authorized operational login.`);
      err.code = 'UNAUTHENTICATED';
      throw err;
    }
    if (!this.can(action, user)) {
      const err = new Error(`[Authorization Denied] Role "${user.role}" is NOT permitted to perform action "${action}".`);
